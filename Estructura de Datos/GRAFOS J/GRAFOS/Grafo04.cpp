...
determinar el grado de entrada de cada vertice
//determinar la diferencia entre el grado de entrada y salida   GE-GS de vertices  y ordenar
*/
...
int main() {			// programa principal
    ...
    void determinarDiferenciaGrados(GrafoMatriz g, Lista *l);  // prototipo de la funcion que determina la diferencia de grados
    ...
    determinarDiferenciaGrados(g,&l);  // llamada a la nueva funcion
    cout << endl<<"Ordenar la diferencia de Grados GE-GS";
...
void determinarDiferenciaGrados(GrafoMatriz g, Lista *l) {
    cout << endl << "Diferencia entre el grado de entrada y salida de los vertices del Grafo";

    for(int i = 0; i < g.getMaxVerts(); i++) {
        int gradoSalida = 0;   // contador de arcos de salida
        int gradoEntrada = 0;  // contador de arcos de entrada

        // Contar arcos de salida
        for (int j = 0; j < g.getMaxVerts(); j++) {
            if (g.getArco(i, j) == 1) {
                gradoSalida++;
            }
        ...
        // Contar arcos de entrada
        for (int j = 0; j < g.getMaxVerts(); j++) {
            if (g.getArco(j, i) == 1) {
                gradoEntrada++;
            }
        }
        int dif = gradoSalida - gradoEntrada;
        l->insertarEnOrden(dif);
        cout << endl << "El vertice " << g.getVertice(i).getDato() << " tiene una diferencia GE - GS de: " << dif;
    }
