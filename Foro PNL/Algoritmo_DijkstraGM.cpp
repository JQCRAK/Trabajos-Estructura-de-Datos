...
void dijkstra(GrafoMatriz& g, int origen) {
    int n = g.getMaxVerts();
    int dist[n]; // array para almacenar distancias más cortas desde el origen
    bool visitado[n]; // array para marcar los vértices visitados
    int prev[n]; // array para almacenar el camino más corto
